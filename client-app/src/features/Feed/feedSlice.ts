...
function getAxiosParams(feedParams: MentionParams) {
   ...
   if(feedParams.datasources.length > 0) params.append('datasources', feedParams.datasources.toString());
   if(feedParams.markets.length > 0) params.append('markets', feedParams.markets.toString());
   if(feedParams.regions.length > 0) params.append('regions', feedParams.regions.toString());
   if(feedParams.products.length > 0) params.append('products', feedParams.products.toString());
   if(feedParams.languages.length > 0) params.append('languages', feedParams.languages.toString());
   return params;
}

export const fetchFeedsAsync = createAsyncThunk<Mention[], void, {state: RootState}>(
    'feed/fetchFeedsAsync',
    async (_, thunkAPI) => {
...
export const feedSlice = createSlice({
    ...
    reducers: {
        setFeedParams: (state, action) => {
            ...
            state.feedParams = {...state.feedParams, ...action.payload, pageNumber: 1};
        },
...
export const feedSelectors = feedsAdapter.getSelectors((state: RootState) => state.feed);
export const {setFeedParams, resetFeedParams, setMetaData, setPageNumber} = feedSlice.actions;